% envtype([[X,poly(T)|_]|_],X,TR) :- copy_term(T, TR).
envtype([[X,T|_]|_],X,T).
envtype([[Y|_]|R],X,T) :- Y \= X, envtype(R,X,T).
...
typeof(Env,let(X,E1,E2),T) :- typeof(Env,E1,TX), typeof([[X,TX]|Env],E2,T).
typeof(Env,letrec(X,E1,E2),T) :- typeof([[X,TX]|Env],E1,TX), typeof([[X,TX]|Env],E2,T).
